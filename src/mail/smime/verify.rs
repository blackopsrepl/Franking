...
use super::{parse_pkcs7, SmimeSigner, SmimeVerification};
...
pub fn verify_detailed(
    ...
    let described = signers
        .iter()
        .map(|cert| describe_signer(cert, &store, verified))
        .collect();
...
pub fn verify_mime_detailed(raw: &[u8], trusted: &[X509]) -> Option<SmimeVerification> {
...
/// Build the trust store for a set of certificates.
fn trust_store(trusted: &[X509]) -> Option<X509Store> {
...
fn describe_signer(cert: &X509Ref, store: &X509Store, verified: bool) -> SmimeSigner {
    let trusted = verified && certificate_chains(cert, store);
    SmimeSigner {
...
fn certificate_chains(cert: &X509Ref, store: &X509Store) -> bool {
    ...
    context
        .init(store, cert, &certs, |ctx| ctx.verify_cert())
        .is_ok()
}
