...
| Keep related threads together without merging them | `Ctrl+o` opens account-scoped collections: toggle membership, create, rename, delete, and filter the list by a collection | A conversation can belong to several collections |
| Locally join conversations without changing recipients' threads | `Ctrl+t` marks a conversation, then merges the next one into it; merged conversations share a thread key and grouping | Purely local grouping; the messages and outbound reply headers are unchanged |
